...
public final class UnsafeClassLoader extends ClassLoader{

	private static final UnsafeClassLoader systemLoader = new UnsafeClassLoader();
	public static UnsafeClassLoader getSystemClassLoader() { //Override
		return systemLoader;
	}
	@Deprecated
	@Override
	...
	public synchronized byte[] getByteCode(String name) throws IOException, NoSuchFieldException, IllegalAccessException {
		...
		try {
			String classFile2 = "/" + UnsafeClassLoader.class.getName().replace('.', '/') + ".class";
			...
			//return null;
			URL url = UnsafeClassLoader.class.getResource(classFile2);
			assert url != null;
